...
exports.likePost = async (req, res) => {
  ...
  try {
    ...
    likedPosts = await student.get("likedPosts");
    let studentContent;
    if (!likedPosts.includes(post_id)) {
      studentContent = await studentModel.findByIdAndUpdate(
        student_id,
        { $push: { likedPosts: post_id } },
        { new: true }
      );
      resMessage = "student added post like.";
    } else {
      studentContent = await studentModel.findByIdAndUpdate(
        student_id,
      ...
      resMessage = "student removed post like.";
    }
    return res.status(201).json({
      message: resMessage,
