...
exports.completeSearch = async function (req, res) {
  try {
    ...
    for (let i = 0; i < searchResultClubs.length; i++) {
      resultsList.push({
    ...
    for (let i = 0; i < searchResultStudents.length; i++) {
      resultsList.push({
        name: searchResultStudents[i].firstName ? searchResultStudents[i].firstName : searchResultStudents[i].name,
